...
# Reference to SciPy/NumPy Package for optimized Dijkstra-Algorithm and matrix-handling
...
# Objective Composition (C1,C2,C3)
SCORE_WEIGHTS = (0.33, 0.33, 0.33)
TRANSFER_PENALTY = 1.25


def create_topology(n_nodes, dx, dy):
    # Create random topology with n_nodes stops and a size of(dx,dy)
    # Feasible Connection are determined using a threshold for the euclidian distance
...
def create_demand_matrix(graph_data, max_node_population):
    # Create random OD-Matrix using a simplified gravity model
...
def create_path_from_prev(prev):
    # Create Paths using predecessor-matrix resulting from shortest-path-algorithm
...
def yen_ksp(graph, k):
    # Create up to k unique paths for each OD-pair
    # Reference to Yen, J. Y. (1971). “Finding the K shortest loopless paths in a network.”

    dist, prev = shortest_path(graph, 'D', return_predecessors=True)  # Shortest Path using Dijkstra from SciPy
    ...
    for i in range(0, len(graph)):
        for j in range(0, len(graph)):
            if i == j:
                continue
            b = []
            # The path generation will be performed for each pair of nodes by isolating specific nodes from the
            # initially calculated shortest path and recalculation using Dijkstra
            for r in range(1, k):
                for x in range(0, len(res[i][j][-1])-1):
                    ...
                    for root_node in root_path:
                        if root_node != spur_node:
                            graph_copy[root_node][:] = float('inf')  # Isolate node in graph
                            graph_copy[:][root_node] = float('inf')
                    dist, prev = shortest_path(graph_copy, 'D', return_predecessors=True)  # Recalculate paths
                    ...
                    cost = 0
                    for n in range(1, len(total_path)):  # evaluate performance measure of path
...
def icrsgp(graph, min_node, max_node, k):
    # create initial solution space considering constraints(min_node,max_node and setting parameter (k)
    # While k determines the unfiltered solution space, the constraints will filter non-feasible elements out.
    ...
    for i in range(0, len(graph)):
        for j in range(0, len(graph)):
            for r in range(0, len(tmp[i][j])):
                if min_node <= len(tmp[i][j][r]) <= max_node:  # Filter paths
...
class Vehicle:
    # Container for holding vehicle characteristics
    # velocity can be used depending on type of topology description
    capacity = 0
    velocity = 0
...
def calculate_solution_stats(graph, solution):
    # Calculate necessary data for processing a route set.
    # Transfer Routes consider a single transfer trip
    # [routes/transfer]_graph := only edges defined in route are not inf
    # [routes/transfer]_dist := minimum cost of travel from origin to destination
    # [routes/transfer]_paths := (combined) path calculated by Dijkstra
...
def populate_routes(routes_dist, routes_paths, routes_length, transfer_dist, transfer_paths, demand, solution):
    # Assign demand/flow to edges in correlation to the shortest paths from origin to destination
    # In case of multiple equally suitable routes, the demand will be distributed in relation to the route length
    # Remaining Demand will be checked against possible 1-Transfer paths and accordingly distributed
    # Demand not covered at this point will be regarded as unfulfilled demand
    ...
    for i in range(0, len(solution)):
        route_population[i] = numpy.max(routes_edge_load[i])  # edge with greatest load determines route demand
    return route_population, dynamic_demand.sum()


def evaluate_user_score(routes_graph, routes_dist, demand):
    # User Score is calculated using fulfillable demand and their respective distances
...
def evaluate_operation_score(routes_dist, routes_paths, routes_length,
    ...
    # Determine bus frequencies, unfulfilled demand and the resulting Operation Score
    ...
    routes_frequency = [max(1, r/vehicle.capacity) for r in routes_load]
...
def main(topology_path="", demand_path=""):
    if topology_path=="" or demand_path == "":
        graph_data = create_topology(15, 1000, 1000)
        demand = create_demand_matrix(graph_data, 100)
    else:
        graph_data = numpy.loadtxt(topology_path, delimiter=",")
        demand = numpy.loadtxt(demand_path, delimiter=";")
